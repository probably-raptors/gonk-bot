...
class WatchCog(commands.Cog):
    def __init__(self, bot: commands.Bot):
        self.channel_id = (
            CONFIG["WATCH_CHANNEL"] if CONFIG["BOT_LIVE"] else CONFIG["DEV_CHANNEL"]
        )
        self.bot = bot
    ...
    async def watchcli(self, ctx, mode=None, *args):
        ...
        if ctx.channel.id != self.channel_id:
            await ctx.channel.send(
                f"Please send watch commands in <#{ self.channel_id }>"
            )
            return
    ...
    async def watch_help(self, ctx, *args):
        ...
        for cmd in self.dispatch.keys():
            await ctx.channel.send(
                f'**{ cmd.lower() }**: *{ self.dispatch[cmd]["info"] }*'
            )

        await ctx.channel.send(
            f'For more information on a given command, please use `{ CONFIG["PREFIX"] }watch COMMAND help`'
        )
        return

    async def add_token(self, ctx, *args):
        m = re.match(
            rf"([a-zA-Z0-9]+) (?:\$)?(\d+(?:\.\d+)?)(?: (?:\$)?(\d+(?:\.\d+)?))?$",
            " ".join(args),
        )
        if m is None:  # show help
            embed = discord.Embed(
                title="Help",
                description="add accepts 3 positional arguments: `SYMBOL PRICE PRICE`",
            )

            # \u200b for "no name field"
            embed.add_field(
                name="Formatting",
                value=f"`{ CONFIG['PREFIX'] }watch add SYMBOL [$]0[.00][ [$]0[.00]]`",
                inline=False,
            )
            embed.add_field(
                name="How it works",
        ...
        if prices["stat"] != 0:
            embed = discord.Embed(
                title=f"Error: { prices['stat'] }",
                description=prices["msg"],
                color=0xA33B24,
            )
            await ctx.channel.send(embed=embed)
            return
        ...
        if (ubnd is not None and utils.atof(ubnd) <= 0) or (
            lbnd is not None and utils.atof(lbnd) <= 0
        ):
            embed = discord.Embed(
                color=0xA33B24,
                title=f"Price Error",
        ...
        sql = (
            "INSERT INTO watch_cog (symbol, user, lower, upper) VALUES (%s, %s, %s, %s)"
        )
    ...
    async def del_token(self, ctx, *args):
        ...
        for (
            ...
            args
        ):  # hacky way of looking up arg[n] as ID or SYMBOL to avoid complex regex parsing
        ...
        if len(ids) == 0:  # HELP
            embed = discord.Embed(
                title="Help",
                description="del accepts an arbitrary number of IDs and SYMBOLS: `(ID|SYMBOL)[ (ID|SYMBOL)...]`",
            )

            # \u200b for "no name field"
            embed.add_field(
                name="Formatting",
                value=f"`{ CONFIG['PREFIX'] }watch del (ID|SYMBOL)[ (ID|SYMBOL)...]`",
                inline=False,
            )
            embed.add_field(
                name="How it works",
        ...
        for id in ids:
            cur.execute(sql, (ctx.author.id, id))
        ...
        for symbol, deleted in info.items():
            ...
            for d in deleted:
                s = f"`ID: { d['id'] }"
                if d["upper"]:
                    s += f" | Upper Bound: { self.format_price(d['upper']) }"
                if d["lower"]:
                    s += f" | Lower Bound: { self.format_price(d['lower']) }"
                s += "`"
                v.append(s)
        ...
        if len(err):
            embed = discord.Embed(
                title=f"Error: Could not delete the following",
                description="\n".join(err),
                color=0xA33B24,
            )
            await ctx.channel.send(embed=embed)
    ...
    async def list_token(self, ctx, *args):
        if len(args) != 0:  # HELP
            embed = discord.Embed(
                title="Help", description="list does not accept any arguments."
            )

            # \u200b for "no name field"
            embed.add_field(
                name="Formatting",
                value=f"`{ CONFIG['PREFIX'] }watch list`",
                inline=False,
            )
            embed.add_field(
                name="How it works",
        ...
        for symbol, info in data.items():
            ...
            for i in info:
                s = f"`ID: { i['id'] }"
                if i["upper"]:
                    s += f" | Upper Bound: { self.format_price(i['upper']) }"
                if i["lower"]:
                    s += f" | Lower Bound: { self.format_price(i['lower']) }"
                s += "`"
                v.append(s)

            embed.add_field(
                name=f"\n{ symbol }  --  { self.format_price(prices['data'][symbol]) }",
                value="\n".join(v),
                inline=False,
            )
    ...
    async def token_info(self, ctx, *args):
        if len(args) == 0 or args[0].lower() == "help":
            embed = discord.Embed(
                title="Help",
                description="Info accepts a space seperate list of SYMBOLS to return info on.",
            )

            # \u200b for "no name field"
            embed.add_field(
                name="Formatting",
                value=f"`{ CONFIG['PREFIX'] }watch info SYMBOL[ [SYMBOL]...]`",
                inline=False,
            )
            embed.add_field(
                name="How it works",
        ...
        for symbol, info in data["data"].items():
            ...
            embed.add_field(
                name="24 Hour Volume", value=self.format_price(info["vol24"])
            )
            embed.add_field(
                name="1 Hour Change", value=self.format_percent(info["chng_1h"])
            )
            embed.add_field(
                name="24 Hour Change", value=self.format_percent(info["chng_24h"])
            )
            embed.add_field(
                name="7 Day Change", value=self.format_percent(info["chng_7d"])
            )
    ...
    async def check_prices(self):
        ...
        if prices["stat"] != 0:
            embed = discord.Embed(
                title=f"Error: { prices['stat'] }",
                description=f"While checking targets, I ran into an issue.\n\n{ prices['msg'] }",
                color=0xA33B24,
            )
        ...
        for r in ret:
            ...
            if symbol in prices["data"].keys():  # more readable price lookup
                ...
                if user not in user_map.keys():
                    user_map[user] = {}
                if symbol not in user_map[user].keys():
                    user_map[user][symbol] = []

                user_map[user][symbol].append(
                    f'Now **{ "over" if above else "under" }** { self.format_price(utils.atos(bound)) }'
                )
        ...
        for user, data in user_map.items():
            ...
            for symbol, msgs in data.items():
                for msg in msgs:
                    embed.add_field(
                        name=f"{ symbol }  --  { self.format_price(prices['data'][symbol]) }",
                        value=msg,
                        inline=False,
                    )
    ...
    def fetch_info(self, symbols):
        ...
        for symbol, info in data["data"].get("data", {}).items():
            ...
            chng_1h = (
                info.get("quote", {}).get("USD", {}).get("percent_change_1h", None)
            )
            chng_24h = (
                info.get("quote", {}).get("USD", {}).get("percent_change_24h", None)
            )
            chng_7d = (
                info.get("quote", {}).get("USD", {}).get("percent_change_7d", None)
            )
            ...
            if (
                ...
                logger.error(
                    f"Parsing error: recieved None for item on symbol '{ symbol }': { info }"
                )
                ret["stat"] = -1
                ret["msg"] = "Failed to parse price information, please try again."
                return ret
